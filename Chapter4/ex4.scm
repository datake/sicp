...
;;;
;;; modified on July 6, 2014 by igarashi
;;; modified on July 15, 2014 by igarashi
;;; modified on July 23, 2014 by igarashi
;;;
;;; Sec 4.1.1
(define (eval exp env cont)
  (cond
   ...
   ((or? exp) (eval-or (or-choices exp) env cont))
...
(define (apply-cont cont val)
  (cond ((haltc? cont) (list 'normal val))
	...
	((orc? cont)
	 (if (true? val) (apply-cont (orc-cont cont) val)
	     (eval-or (orc-exps cont) (orc-env cont) (orc-cont cont))))
;;; Changes for CATCH-THROW begins
	((cabodyc? cont)
...
;;;変更 tagにあたるものを(cons tag handler)に変更する
	...
	((throwc? cont)
	 (let ((stripped-cont
	   ...
	   ;;;     (apply-cont stripped-cont val)
	       ...
	       (define applyc-tag-handler (applyc-cont (consc-cont (restopsc-cont (throwc-cont cont)))))
					;handlerの定義
	       (define handler (catchc-handler applyc-tag-handler))
	       (my-apply 
		(make-procedure (lambda-parameters handler) (lambda-body handler) the-global-environment)
		(list val) applyc-tag-handler) 
...
(define (make-orc exps env cont) (list 'orc exps env cont))
(define (orc? cont) (tagged-list? cont 'orc))
(define (orc-exps cont) (cadr cont))
(define (orc-env cont) (caddr cont))
(define (orc-cont cont) (cadddr cont))
...
(define (cabodyc-cont cont) (cadddr cont))
					...
					;変更　tag をtag-handler-pair に変更
(define (make-catchc tag-handler-pair cont) (list 'catchc tag-handler-pair cont))

(define (catchc? cont) (tagged-list? cont 'catchc))
					;(define (catchc-tag cont) (cadr cont))

					;変更　catchc-tag はtagとhandlerのペアのタグをさす
