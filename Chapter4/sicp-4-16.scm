					...
					;手続き本体をとり、defineをlambda とletで表す手続き
(define true #t)
(define false #f)

					;4.16 scan-out-definesは手続き本値をとる
(define (scan-out-defines body)
					;exp:式 vars:変数列 sets: exps:
  (define (iter exp vars sets exps)
    (if (null? exp)
					;expがnullの場合
	(list (reverse vars) (reverse sets) (reverse exps))
					;expがnullでない場合、expのタグがdefineかどうか
	(if (definition? (car exp))
					;exp のタグがdefineであったとき
	    (iter 
	     (cdr exp) ;next-exp
	     (cons (list (definition-variable (car exp)) "*unassigned*") vars) ;next-vars
	     (cons (list 'set! 
			 (definition-variable(car exp)) 
			 (definition-value (car exp)))
		   sets) ;next-sets
	     exps) ;next-exps
					;expのタグがdefineでないときvars,setsはそのままでexpの先頭をexpsの先頭に結合させiterに渡す)
    ...
    (define (scan vars vals)
      (cond ((null? vars)
	     ...
	     (if (eq? '*unassigned* (car vals))
					;a:(car vals)が*unsigned*ならエラーを返す   
...
;[moved to start of file] (define apply-in-underlying-sch
