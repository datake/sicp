...
(define (prod-list list)
  (define (prod-list-iter list product)
    (cond    ((eq? (car list) 0)(throw 'answer-is-zero 0) )
	     ((null? list)product)
	     (else
	      (prod-list-iter (cdr list) (* (car list) product)
			      )
	      )
	     )
    )
  (catch 'answer-is-zero (prod-list-iter list 1)
